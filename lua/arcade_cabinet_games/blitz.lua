...
local function ResetClouds()
    ...
    local function BombExplodeCars(x1,y1)
        for i = 1, #cars do
            if cars[i].alive == 1 and cars[i].destroyed == 0 and CheckCollision(cars[i].x + 20,cars[i].y,x1 - 40,y1 - 40,80,80) then
                cars[i].destroyed = 1
                points = points + 500
                SOUND:EmitSound("ambient/office/tech_oneshot_08.wav", 40)
            end
...
function GAME:Update()
    ...
    if state == 9 and lastUpdate + 0.020 < RealTime() then -- no player screen
        lastUpdate = RealTime()
    ...
    if state == 0 then -- in game
        if lastUpdate + 0.020 < RealTime() then
            ...
            if thePlayer:KeyDown(IN_JUMP) and not thePlayer:KeyDown(IN_BACK) and nuke.levels == 0 and nuke.alive == 0 and bomb.alive == 0 then
                DropBomb()
                SOUND:EmitSound("ambient/office/slidechange.wav", 40)
            end

            if thePlayer:KeyDown(IN_JUMP) and thePlayer:KeyDown(IN_BACK) and nuke.can_drop == 1 then
...
function GAME:Draw()
    ...
    if gameState == 0 then
        ...
        surface.SetFont("DermaLarge")
        tw, th = surface.GetTextSize("INSERT COIN")
